...
export default class ProyectosIndependienteVista extends Component {
	...
	getJobs = (user) => {
		return fetch(url + 'fixpertoEmpresa/getJobsEmpresa', {
			method: "POST",
			...
			if (responseJson.success) {
				...
				for (let index = 0; index < responseJson["jobs"].length; index++) {
					jobs.push({
						uri: url + "uploads/registros/empresa/jobs/" + responseJson["jobs"][index]["job"],
	...
	adicionar = () => {
		...
		if (vacios.length) {
			...
			return fetch(url + 'fixpertoEmpresa/addJobEmpresa', {
	...
	render() {
		...
		return (
			<View style={{ flex: 1, backgroundColor: "#FFFFFF" }}>
				...
				<SafeAreaView style={{ marginBottom: 10, marginHorizontal: 10, alignItems: "center" }}>
