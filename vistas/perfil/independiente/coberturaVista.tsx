...
import { View, ScrollView, Dimensions, AsyncStorage, Image, Text } from 'react-native';
...
export default class CoberturaIndependienteVista extends Component {
	multiSelect = null;
	constructor(props) {
		super(props);
	...
	getCobertura = () => {
		AsyncStorage.getItem("@USER").then((user) => {
			...
			return fetch(url + 'fixpertoProfesional/getCobertura', {
				method: "POST",
	...
	saveRegion() {
		if (this.state["selectedRegions"].length > 0) {
			AsyncStorage.getItem("@USER").then((user) => {
				user = JSON.parse(user);
				return fetch(url + 'fixpertoProfesional/modCobertura', {
					method: "POST",
					headers: { Accept: 'application/json', 'Content-Type': 'application/json', "Access-Token": globalThis.tokenAuth },
