...
export default class AgendadosOfertaVista extends Component {
	...
	getOfferts = () => {
		...
		AsyncStorage.getItem("@USER").then((user) => {
			...
			return fetch(url + 'fixperto/getOffertsScheduleds', {
				method: "POST",
			...
			<View style={{ flex: 1, backgroundColor: "#FFFFFF" }}>
				<ScrollView style={{ backgroundColor: "#FFFFFF" }}>
					...
					<View style={{ backgroundColor: "#EAF9D9", padding: 20 }}>
						<Text style={{ color: "#86CA3D", textAlign: "center", fontFamily: "Raleway-Bold" }}>{(this.state["services"].length) ? "Tus" : "No hay"} trabajos programados</Text>
					</View>
					{
										...
										<View style={[general.cont]}>
											...
											<View style={[general.cont_parte2]}>
												...
												<View style={[fixperto.text_exp]}>
													<Text style={{ flex: 1, marginStart: 5 }}>{item.customer}</Text>

													<Text style={{ flex: 1, marginStart: 5 }}>Fecha del servicio: {item.start_date}</Text>

													<TouchableOpacity style={[general.cont2, general.cont4]}
														onPress={() => this.props["navigation"].navigate("AgendadoOferta", { item })}>
					...
					<Cargador show={this.state["cargador"]} texto="Actualizando servicios agendados..." />
				</ScrollView>
